...
<body x-data="{ page: 'doc_index', 'loaded': true, 'darkMode': false, 'stickyMenu': false, 'sidebarToggle': true, 'scrollTop': false, 'isDeleteNewFolderModal': false, 'isAddEditFolderModal': false, 
    ...
    @if (Session('link'))
        <div x-init="
                    ...
                    }" data-link-link="{{ session('link') }}" data-link-name="{{ session('file') }}"
            data-link-expires-day="{{ session('expires_day') }}" data-link-expires-time="{{ session('expires_time') }}">
        </div>
    @endif
    <!-- ===== Page Wrapper Start ===== -->
                        ...
                        <nav>
                            <ol class="flex items-center gap-1.5">
                                ...
                                <li class="inline-flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400">
                                    <a href="{{ route('dashboard.all') }}">Documentos</a>
                                </li>
                            </ol>
                        </nav>
                    </div>
                    <div class="w-1/2 h-full flex items-center justify-end">
                        ...
                        <form method="GET" action="{{ route('dashboard.all') }}" class="ml-3 flex items-center gap-2">
                            ...
                            <select name="sort" class=" cursor-pointer text-sm px-2 py-1 border rounded text-gray-500 dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <option value="name_desc" {{ $currentSort == 'name_desc' ? 'selected' : '' }}>Nome (Z → A)</option>
                                <option value="size_desc" {{ $currentSort == 'size_desc' ? 'selected' : '' }}>Tamanho (maior)</option>
                                <option value="size_asc" {{ $currentSort == 'size_asc' ? 'selected' : '' }}>Tamanho (menor)</option>
                            </select>
                            @php
                                $selectedUploader = isset($filters['uploader']) ? $filters['uploader'] : request('uploader');
                                $selectedEditor = isset($filters['editor']) ? $filters['editor'] : request('editor');
                            @endphp
                            ...
                            <button type="submit" class="inline-flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-white rounded-sm transition bg-brand-500/95 shadow-theme-xs hover:bg-brand-600">Aplicar</button>
                            <a href="{{ route('dashboard.all') }}" class="ml-1 text-sm text-gray-500 dark:text-gray-300">Limpar</a>
                        </form>
                            ...
                            <table class="min-w-full">
                                ...
                                <tbody
                                    ...
                                    <tr @dblclick="window.open('{{ route('files.preview', $file->id) }}', '_blank')"
                                        ...
                                        <td class="px-5 py-4 sm:px-6 max-w-3 w-3 ">
                                            <div class="flex items-center justify-center">
                                                <p
                                                    class=" whitespace-nowrap text-gray-500 text-theme-sm dark:text-gray-400">
                                                    {{ str_pad($loop->iteration + ($files->currentPage() - 1) * $files->perPage(), 3, '0', STR_PAD_LEFT) }}
                                                </p>
                                            </div>
                                        </td>
                                            ...
                                            <div class="flex items-center gap-2">
                                                <div
                                                    ...
                                                    <p class="text-gray-500 text-xs dark:text-gray-400">
                                                        {{ Str::of($file->creator->name)->trim()->explode('
                                                        ')->map(fn($part) => $part[0])->take(2)->join('') }}
                                                    </p>
                                                </div>
                                                <p
                                                    class=" whitespace-nowrap text-gray-500 text-theme-sm dark:text-gray-400">
                                        ...
                                        <td class="px-5 py-4 sm:px-6">
                                            <div
                                                ...
                                                @if (auth()->user()->id == $file->created_by || auth()->user()->role ==
                                                'admin')
                                                <button @click="
                                                ...
                                                @else
                                                <button
                                                    class=" opacity-50 cursor-not-allowed h-7 w-7 flex items-center justify-center rounded-full hover:bg-gray-300 dark:hover:bg-gray-900">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                        class="size-3.5">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                ...
                                                @if (now() < $file->expires_at)
                                                    ...
                                                    @if (auth()->user()->id == $file->created_by || auth()->user()->role
                                                    == 'admin')
                                                    <div @click="isDeleteFileModal = true; deleteFileData.id = $el.dataset.fileId; deleteFileData.name = $el.dataset.fileName; deleteFileData.createdBy = $el.dataset.fileCreatedBy;"
                                                        data-file-id="{{ $file->id }}"
                                                        data-file-name="{{ $file->name }}"
                                                    ...
                                                    @else
                                                    <div
                                                        class=" opacity-50 cursor-not-allowed h-7 w-7 flex items-center justify-center rounded-full hover:bg-red-500 hover:text-white dark:hover:bg-red-500 dark:hover:text-white">
                                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                            class="size-4">
                                                            <path stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <tr @dblclick="window.open('{{ route('files.preview', $file->id) }}', '_blank')"
                                        class=" opacity-50 hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer">
                                        <td class="px-5 py-1 sm:px-6">
                                            <div class="flex items-center">
                                                <div class="flex items-center gap-3">
                                                    <div>
