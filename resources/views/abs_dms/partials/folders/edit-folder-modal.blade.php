<div x-show="isAddEditFolderModal" class="fixed inset-0 flex items-center justify-center p-5 overflow-y-auto z-99999">
  <div class="modal-close-btn fixed inset-0 h-full w-full bg-gray-400/25 backdrop-blur-[1px]"></div>
  <div @click.outside="isAddEditFolderModal = false"
    ...
    <!-- close btn -->
    <button @click="isAddEditFolderModal = false"
    ...
    <form class="flex flex-col" x-bind:action="'{{ route('folders.update', '__ID__') }}'.replace('__ID__', editFolderData.id)" method="POST">
      @method('PUT')
      @csrf
      <input type="hidden" name="id" x-model="editFolderData.id">
      <div class="flex flex-col gap-5 px-2 overflow-y-auto custom-scrollbar">
        <div class="flex flex-col gap-x-6 gap-y-5">
          <div>
            ...
            <input type="text" name="name" x-model="editFolderData.name"
          ...
          <div>
            ...
            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
              <select x-model="editFolderData.tag" name="tag"
      ...
      <div class="flex items-center gap-3 mt-6 lg:justify-end">
        <button @click="isAddEditFolderModal = false" type="button"
